...
function findOrCreateUserForOauthResponse(provider, accessToken, token_secret, user, profile, done) {
  if (user) {
    ...
    if (service_token.token != accessToken || service_token.display_name != profile.displayName) {

      if (provider == "instagram") {
        user.instagram_id = profile.id;
        user.instagram_name = profile.username;      
        user.bio = profile._json.data.bio;
        user.website = profile._json.data.website;
        user.external_counts.instagram = profile._json.data.counts;
        user.profile_picture = profile._json.data.profile_picture;      
        user.oauth_token = accessToken;
      }
      user.set_token_from_profile(provider, profile, accessToken, token_secret);
...
passport.use(new InstagramStrategy({
  ...
  function(accessToken, refreshToken, profile, done) {
    ...
    process.nextTick(function () {
      User.findOne().or([{"tokens.instagram.account_id": profile.id}, {"instagram_id": profile.id}]).run(function(err, user){
        if (err) throw err;
...
passport.use("instagram-connect", new InstagramStrategy({
  ...
  function(accessToken, refreshToken, profile, done) {
    User.findOne({"tokens.instagram.account_id": profile.id}, function(err, user){
      ...
      if (account.display_name == null || acount.display_name == "") {
